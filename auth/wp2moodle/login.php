...
function update_user_license_info($userid, $new = false) {
    ...
    $req = array(
        'uid' => $user_info->idnumber
    );
    file_put_contents(__DIR__ . '/returned_user.txt', print_r($req, true));
    ...
    */
    ...
    // decode the result
    $license_info = json_decode($result);
    file_put_contents(__DIR__ . '/license_info.txt', print_r($license_info, true));
    // success?
...
if (!empty($_GET)) {
	...
	if ($timestamp > 0 && $diff <= $timeout) { // less than N minutes passed since this link was created, so it's still ok
		...
		if ($DB->record_exists('user', array('username'=>$username, 'idnumber'=>'', 'auth'=>'manual'))) { // update manually created user that has the same username but doesn't yet have the right idnumber
			...
			update_user_license_info($updateuser->id);
